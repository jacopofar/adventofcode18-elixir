defmodule DetectSeenPartialSums do
    ...
    partial sum, as if it was infinitely concatenaded with itself.
    """
    def detect(filename) do
        ...
        |> Stream.transform(%{
            :current_total => 0,
            :seen_total => MapSet.new([0])},
            fn (candidate_line, acc) ->
            case candidate_line do
                {number, _} ->
                    if MapSet.member?(acc[:seen_total], acc[:current_total] + number) do
                        ...
                        {[candidate_line],
                         %{:current_total => acc[:current_total] + number,
                         :seen_total => MapSet.put(
                             acc[:seen_total], acc[:current_total] + number)}}
                    end
                    _ ->
