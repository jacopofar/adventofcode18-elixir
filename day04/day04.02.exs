defmodule FindSleepyGuard do
    ...
    Events has to be sorted by timestamp.

    A guard may have no events, or multiple sleep and awake

    Calculate which guard has the minute with highest sleep count.
    """
    defp guardId(s) do
    ...
    def guardWithSleepiestMinute(sleep_map) do
...
{sleepy_guard_id, sleepy_guard_sleep_frequency} = FindSleepyGuard.processFile("day04.txt")
    |> Enum.to_list
    |> List.last
    |> FindSleepyGuard.guardWithSleepiestMinute
